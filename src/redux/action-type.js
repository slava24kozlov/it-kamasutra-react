//ProfileReducer
export const SET_POST = 'SET-POST';
export const UPDATE_POST_TEXT = 'UPDATE-POST-TEXT';
export const UPDATE_COUNT_LIKE = 'UPDATE-COUNT-LIKE';
export const SET_PROFILE = 'SET-PROFILE';
export const SET_STATUS = 'SET-STATUS';

//MessagesReducer
export const SET_MESSAGE = 'SET-MESSAGE';

//UsersReducer
export const FOLLOW = 'FOLLOW';
export const UNFOLLOW = 'UNFOLLOW';
export const SET_USERS = 'SET-USERS';
export const SET_CURRENT_PAGE = 'SET-CURRENT-PAGE';
export const SET_TOTAL_USERS_COUNT = 'SET-TOTAL_USERS_COUNT';
export const IS_FETCHING_USERS = 'IS-FETCHING-USERS';
export const IS_FOLLOWING_USERS = 'IS-FOLLOWING-USERS';

//auth-reducer
export const SET_AUTH_USERS = 'SET-AUTH-USER';
export const SET_AUTH_LOGIN_DATA = 'SET-AUTH-LOGIN-DATA';
export const SET_AUTH_RESPONSE_MESSAGE = 'SET-RESPONSE-MESSAGE';


